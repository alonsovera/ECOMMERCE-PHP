...
<head>
    ...
    <script src="https://www.paypal.com/sdk/js?client-id=AbaC35oMR7Anb2SRxbM9OYFWHn0kH7WYTW5tAOHEGGPRlaIbYL33SkvhBxU0rAOU0EJF-E_8TS_wFfHh&currency=USD"></script>
...
<body>
    ...
    <h3 class="text-center">Carrito de compras</h3>
        ...
        <div class="container mt-4" id = "compra">
            <div class="table-responsive">  <!-- Envoltura responsive para la tabla -->
                <table class="table">
                    <thead>
                        <tr>
                            <th>Imagen</th>
                            <th>Producto</th>
                            <th class="d-none d-md-table-cell">Descripción</th> <!-- Ocultar en tamaños sm -->
                            <th>Cantidad</th>
            ...
            <button onclick="realizarPago()" class="btn btn-success">Realizar Pago</button>
            <button onclick="cancelarPago()" class="btn btn-danger" style="display: none;">Cancelar</button>
          
</div>
  <center>
        <img id="imagenCarritoVacio" src=".././Content/image/carrito_vacio.png" style="display: none;" />
        </center>
    ...
    <script>
        ...
        function cargarDetalleCarrito() {
            $.ajax({
                url: '/tf/controllers/Carrito/ListaDetalleCarrito.php',
                type: 'POST',  
                async: false,
                ...
                success: function(data) {
                    if(data === "empty") {
                        $('#compra').hide();  
                        $('#imagenCarritoVacio').show();  
                    } else {
                        $('#detalleCarrito').html(data);
                        $('#detalleCarrito').parent().show();  
                        $('#imagenCarritoVacio').hide();  
                    }
                },
                error: function() {
        ...
        function incrementarCantidad(pkDetalleCarrito) {
                $.ajax({
                    url: '/tf/controllers/Carrito/incrementarCantidad.php',  
                    type: 'POST',
                    async:false,
                    data: { pkDetalleCarrito: pkDetalleCarrito },
                    success: function(response) {
                        console.log(response); 
                    },
                    error: function(xhr, status, error) {
                        console.error('Error al incrementar la cantidad:', xhr.responseText);
                    }
                });
        ...
        function decrementarCantidad(pkDetalleCarrito) {
            $.ajax({
                    url: '/tf/controllers/Carrito/decrementarCantidad.php',  
                    type: 'POST',
                    async:false,
                    data: { pkDetalleCarrito: pkDetalleCarrito },
                    success: function(response) {
                    ...
                    error: function(xhr, status, error) {
                        console.error('Error al decrementar la cantidad:', xhr.responseText);
                    }
                });
                cargarDetalleCarrito();
        }
        ...
        function eliminarProductoDelCarrito(pkDetalleCarrito) {
            $.ajax({ type:"POST",
                        url: "/tf/controllers/Carrito/eliminarProductoDelCarrito.php", 
                            async: false,
                        data:{pkDetalleCarrito:pkDetalleCarrito},
        ...
        function pagoRecibido(){
            $.ajax({
                url: '/tf/controllers/Venta/GenerarVenta.php',
                type: 'POST',
                async: false,
                data: {
                    clienteId: clienteId,        
                ...
                success: function(response) {
                    console.log(response);
                    alert('Pago realizado y detalles enviados');
        ...
        function cargarTotalCarrito() {
            $.ajax({
                url: '/tf/controllers/Carrito/getTotalCarrito.php',
                type: 'POST',
                async: false,
                data: { userCarrito: userCarrito },
                success: function(response) {
                    var total1 = parseFloat(response);
                    var total = total1 * 0.265;
                    console.log(response); 
                    if (!isNaN(total)) {
                        $('#totalCarrito').text(total.toFixed(2)); 
                        configurarBotonesPaypal(total.toFixed(2)); 
                    } else {
                        console.error('Error: el total recibido no es un número.');
                    }
                },
                error: function(xhr, status, error) {
                    console.error('Error al cargar el total del carrito:', xhr.responseText);
                }
            });
        }      
        ...
        function configurarBotonesPaypal(total) {
            paypal.Buttons({
        ...
        function enviarCorreo(total) {
            $.ajax({
                url: '/tf/PhpMailer/send.php',
                type: 'POST',
                async: false,
                data: {
                    userEmail: userEmail,
        ...
        function actualizarContadorCarrito() {
            $.ajax({
                url: '/tf/controllers/Carrito/ContarItemsCarrito.php',  
                type: 'POST',
                async: false,
                data: { userCarrito: userCarrito },
                success: function(response) {
                ...
                error: function(xhr) {
                    console.error('Error al cargar la cantidad del carrito:', xhr.responseText);
                }
            });
        }
